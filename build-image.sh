...
docker build -t aymericbernard/rerum-imperium-backend:$version --build-arg BUILD_CONFIGURATION=$BUILD_CONFIGURATION --build-arg VERSION=$version . || { echo 'build failed' ; exit 1; }

echo "Pushing rerum-imperium-backend:$version to docker registry..."
docker push aymericbernard/rerum-imperium-backend:$version || { echo 'push failed' ; exit 1; }
